...
#ifndef _tuiframework_MSPMatrix4x4Transf_h_
#define _tuiframework_MSPMatrix4x4Transf_h_
...
class MSPMatrix4x4Transf : public IMSP {
public:
    static IMSP * createFunction(void * arg);
    static std::string getMSPTypeName();

    MSPMatrix4x4Transf(const MSPConfig & config);
    virtual ~MSPMatrix4x4Transf();
   ...
   // virtual const MSPInstance & getMSPInstance() const;
    ...
    void transform(Matrix4x4ChangedEvent * event);
